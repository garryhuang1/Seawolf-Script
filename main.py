#Garry Huang 109247416
import sys
from antlr4 import *
from SeawolfScriptLexer import SeawolfScriptLexer
from SeawolfScriptParser import SeawolfScriptParser
from antlr4.error.ErrorListener import ErrorListener
from SeawolfScriptVisitor import SeawolfScriptVisitor
#from MyVisitor import MyVisitor
errorLine = []
errorFound = []
class MyErrorListener( ErrorListener):
	def __init__(self):
		super(MyErrorListener, self).__init__()
	def syntaxError(self, recognizer, offendingSymbol, line, column, msg, e):
		if(line in errorLine) == False:
			errorLine.append(line)
class MyVisitor(SeawolfScriptVisitor):
	def __init__(self):
		self.memory={}
	def visitPrintExpr(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		value = self.visit(ctx.expr())
		if value is None:
			return
		print(value)
		return 0
	def visitNegative(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		value = self.visit(ctx.expr())
		return -1 * value
	def visitId(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		name = ctx.ID().getText()
		if name in self.memory:
			return self.memory[name]
		return 0
	def visitAssignMatrix(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			print("SYNTAX ERROR")
			return
		if(ctx.start.line in errorFound):
			return None
		name = ctx.ID().getText()
		value = self.visit(ctx.expr(0))
		secondValue = self.visit(ctx.expr(1))
		insertValue = self.visit(ctx.expr(2))
		oldValue = self.memory[name]
		oldValue[value][secondValue]=insertValue
		self.memory[name] = oldValue
		return insertValue
	def visitAssignIndex(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			print("SYNTAX ERROR")
			return 
		if(ctx.start.line in errorFound):
			return None
		name = ctx.ID().getText()
		value = self.visit(ctx.expr(0))
		insertValue = self.visit(ctx.expr(1))
		oldValue = self.memory[name]
		oldValue[value]=insertValue
		self.memory[name] = oldValue
		return insertValue
	def visitAssign(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			print("SYNTAX ERROR")
			return 
		if(ctx.start.line in errorFound):
			return None
		name = ctx.ID().getText()
		value = self.visit(ctx.expr())
		self.memory[name] = value
		return value
	def visitArray(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		length = (len(ctx.expr()))
		words = []
		for i in range(0,length):
			words.append(self.visit(ctx.expr(i)))
		return words
	def visitString(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		newStr = str(ctx.STRING().getText())
		newStr = newStr[1:-1]
		return str(newStr)
	def visitReal(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return 
		return float(ctx.REAL().getText())
	def visitInt(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		return int(ctx.INT().getText())
	def visitIn(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = self.visit(ctx.expr(0))
		right = self.visit(ctx.expr(1))
		if left in right:
			return 1
		else:
			return 0
	def visitIndex(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = self.visit(ctx.expr(0))
		if (type(left) is list or type(left) is str) == False:
			return "SEMANTIC ERROR"
		right = self.visit(ctx.expr(1))
		if (type(right) is int) == False:
			return "SEMANTIC ERROR"
		if (right >= len(left)) == True:
			return "SEMANTIC ERROR"
		indexAt = left[right]
		return indexAt
	def visitExponent(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = (self.visit(ctx.expr(0)))
		right = (self.visit(ctx.expr(1)))
		if (type(left) is int or type(left) is float) == False:
			return "SEMANTIC ERROR"
		if (type(right) is int or type(right) is float) == False:
			return "SEMANTIC ERROR"
		return left ** right
	def visitMulDiv(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = (self.visit(ctx.expr(0)))
		right = (self.visit(ctx.expr(1)))
		if (type(left) is int or type(left) is float) == False:
			return "SEMANTIC ERROR"
		if (type(right) is int or type(right) is float) == False:
			return "SEMANTIC ERROR"
		if ctx.op.type == SeawolfScriptParser.MUL:
			return left * right
		if(right == 0):
			return "SEMANTIC ERROR"
		return left / right
	def visitAddSub(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = (self.visit(ctx.expr(0)))
		right = (self.visit(ctx.expr(1)))
		if(type(left) is int or type(left) is float) and (type(right) is int or type(right) is float) == True and ctx.op.type == SeawolfScriptParser.ADD:
			return left + right
		if(type(left) == type(right)) == False:
			return "SEMANTIC ERROR"
		if (type(left) is str and type(right) is str) and ctx.op.type == SeawolfScriptParser.ADD:
			return str(left + right)
		if (type(left) is list and type (right) is list) and ctx.op.type == SeawolfScriptParser.ADD:
			newList = left + right
			return newList
		if ctx.op.type == SeawolfScriptParser.ADD and (type(left) is int and type(right) is int):
			return left + right
		if (type(left) is int or type(left) is float) == False and ctx.op.type == SeawolfScriptParser.SUB:
			return "SEMANTIC ERROR"
		if (type(right) is int or type(right) is float) == False and ctx.op.type == SeawolfScriptParser.SUB:
			return "SEMANTIC ERROR"
		return left - right
	def visitFloor(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = (self.visit(ctx.expr(0)))
		right = (self.visit(ctx.expr(1)))
		if (type(left) is int or type(left) is float) == False:
			return "SEMANTIC ERROR"
		if (type(right) is int or type(right) is float) == False:
			return "SEMANTIC ERROR"
		return left//right
	def visitMod(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = (self.visit(ctx.expr(0)))
		right = (self.visit(ctx.expr(1)))
		if (type(left) is int or type(left) is float) == False:
				return "SEMANTIC ERROR"
		if (type(right) is int or type(right) is float) == False:
			return "SEMANTIC ERROR"
		return left % right
	def visitParens(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		return self.visit(ctx.expr())
	def visitAnd(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = (self.visit(ctx.expr(0)))
		right = (self.visit(ctx.expr(1)))
		if (type(left) is int and type(right) is int) == False:
			return "SEMANTIC ERROR"
		leftBool = True;
		rightBool = True;
		if left == 0:
			leftBool = False
		if right == 0:
			rightBool = False
		
		if leftBool and rightBool == True:		
			return 1
		else:
			return 0
		return -1
	def visitOr(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = (self.visit(ctx.expr(0)))
		right = (self.visit(ctx.expr(1)))
		if (type(left) is int and type(right) is int) == False:
			return "SEMANTIC ERROR"
		leftBool = True;
		rightBool = True;
		if left == 0:
			leftBool = False
		if right == 0:
			rightBool = False
		if leftBool or rightBool == True:
			return 1
		else:
			return 0
	def visitNot(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		num = (self.visit(ctx.expr()))
		if (type(num) is int) == False:
			return "SEMANTIC ERROR"
		if num == 0:
			return 1
		else:
			return 0
		return 0
	def visitCond(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			return "SYNTAX ERROR"
		if(ctx.start.line in errorFound):
			return None
		left = (self.visit(ctx.expr(0)))
		right = (self.visit(ctx.expr(1)))
		if (type(left) is int and type(right) is int) == False:
			return "SEMANTIC ERROR"
		if ctx.op.type == SeawolfScriptParser.LTHAN:
			if(left < right):
				return 1
			else:
				return 0
		if ctx.op.type == SeawolfScriptParser.GTHAN:
			if(left > right):
				return 1
			else:
				return 0
		if ctx.op.type == SeawolfScriptParser.LTHANE:
			if(left<=right):
				return 1
			else:
				return 0
		if ctx.op.type == SeawolfScriptParser.GTHANE:
			if(left>=right):
				return 1
			else:
				return 0
		if ctx.op.type == SeawolfScriptParser.EQUAL:
			if(left==right):
				return 1
			else:
				return 0
		if ctx.op.type == SeawolfScriptParser.NEQUAL:
			if(left!=right):
				return 1
			else:
				return 0
		return 0
	def visitIf(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			print("SYNTAX ERROR")
			return
		if(ctx.start.line in errorFound):
			return None
		evaluatedBlock = self.visit(ctx.expr())
		if(evaluatedBlock == 1):
			self.visit(ctx.stat_block(0))
		elif evaluatedBlock == 0 and ctx.stat_block(1) is not None:
			self.visit(ctx.stat_block(1))
		return 
	def visitWhile(self, ctx):
		if(ctx.start.line in errorLine):
			errorLine.remove(ctx.start.line)
			errorFound.append(ctx.start.line)
			print("SYNTAX ERROR")
			return 
		if(ctx.start.line in errorFound):
			return None
		evaluatedBlock = self.visit(ctx.expr())
		while(evaluatedBlock == 1):
			self.visit(ctx.stat_block())
			evaluatedBlock = self.visit(ctx.expr())
		return
if __name__ == '__main__':
    input = FileStream(sys.argv[1])
    lexer = SeawolfScriptLexer(input)
    lexer.removeErrorListeners()
    lexer._listeners = [MyErrorListener()]
    stream = CommonTokenStream(lexer)
    parser = SeawolfScriptParser(stream)
    parser.removeErrorListeners()
    parser._listeners = [MyErrorListener()]
    tree = parser.prog()
    
    #lisp_tree_str = tree.toStringTree(recog=parser)
    #print(lisp_tree_str)
    visitor = MyVisitor()
    visitor.visit(tree)
   
    